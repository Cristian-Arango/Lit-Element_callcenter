    ...
    mostrarEquipos() {
        ...
        return html`
            <ul>
                ${this.equipos.map(
                    (equipo) => html`
                        <li> NOMBRE: ${equipo.nombre} - NUMERO DE PARTICIPANTES: ${equipo.numero} Campaña: ${equipo.categoria}</li>
                    `
                )}
            </ul>
        `;
        ...
        <div class="pt-2 d-flex justify-content-left ">
            <div class="d-flex  flex-shrink-0 p-3 ml-5" style="width: 250px; background-color: rgb(201, 205, 207); border-radius: 1rem; height: 42.4rem;">
                ...
                <ul class="nav nav-pills flex-column mb-auto" style="width: 25rem;">
                    <li class="nav-item">
                        <button  " class="nav-link active bg-light pt-2 font-weight-bold" style="color: grey; border-radius: 10px; height: 45px; width: 13.5rem; font-size: 18px;" aria-current="page",  @click=${(e)=>this.USUARIOS()}>
                        ...
                        <div>
                            <div class="d-flex  flex-shrink-0 p-3" style="width: 222px; background-color: rgb(201, 205, 207); border-top-left-radius: 1rem; border-top-right-radius: 1rem; height: 15rem; ">
                                ...
                                <ul class="nav nav-pills flex-column mb-auto" style="width: 25rem;">
                                    ...
                                    <input id="numero" class="form-control font-weight-bold2"style="width: 12rem;" type="text" placeholder="numero equipo" />
                                        <select id="opcion">
                                        ${campañasActivas.map(campaña => html`<option>${campaña.nombre}</option>`)}
                                        </select>

                                    <div class="d-flex justify-content-center">
                                        <button class="mt-2 text-center" style=" width: 5rem; border-radius: 5px; border: rgb(250, 101, 101); background-color: rgb(250, 101, 101); color: white", @click=${this.guardarDatosEnLocalStorages}>Agregar Campaña</button>
                                    </div>
                                </ul>
                                <ul id="listaUsuarios">
