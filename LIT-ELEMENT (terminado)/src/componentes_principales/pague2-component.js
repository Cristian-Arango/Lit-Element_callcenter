...
export class Index2 extends LitElement {
    ...
    constructor() {
        ...
        this.resultadosBusqueda = this.usuarios; // Inicialmente, muestra todos los usuarios.
 ...
 toggleCampañaActiva(campaña) {
    ...
    actualizarUsuarios(resultados = this.resultadosBusqueda) {
        ...
        if (resultados.length === 0) {
            ...
            resultados.forEach((usuario) => {
                ...
                llamarButton.addEventListener('click', () => this.abrirModalLlamda(usuario));
    ...
    obtenerCampañasDesdeLocalStorage() {
        ...
        const campañasJSON = localStorage.getItem("campañas");
        return campañasJSON ? JSON.parse(campañasJSON) : [];
        console.log(campañasJSON)
    }
    renderUsuarios() {
        ...
        return html`
            <ul>
                ${this.resultadosBusqueda.map(
                    (usuario) => html`
                        <li>
                            ${usuario.nombre} - ${usuario.email} - ${usuario.documento}-
                        </li>
                    `
                )}
            </ul>
    ...
    filtrarPorCampaña(event) {
        ...
        console.log('Campaña seleccionada:', campañaSeleccionada); // Agrega este log para verificar la campaña seleccionada
    
        if (campañaSeleccionada === "") {
            // Si no se selecciona ninguna campaña, muestra todos los usuarios
  ...
  render() {
    const campañasActivas = this.campañas.filter(campaña => campaña.activo);
        ...
        <div class="d-flex  flex-shrink-0 p-3 ml-5" style="width: 250px; background-color: rgb(201, 205, 207); border-radius: 1rem; height: 42.4rem;">
            ...
            <ul class="nav nav-pills flex-column mb-auto" style="width: 25rem;">
                ...
                <li class="nav-item pt-3">
                    <button " class="nav-link active bg-light pt-2 font-weight-bold" style="color: grey; border-radius: 10px; height: 45px; width: 13.5rem; font-size: 18px;" aria-current="page",  @click=${(e)=>this.campaña()}>
                        <span class="material-symbols-outlined" id="p">
                        person_apron
                        </span> Campañas
                    </button>
                ...
                <li class="nav-item pt-3">
                    <button class="nav-link active bg-light pt-2 font-weight-bold" style="color: grey; border-radius: 10px; height: 45px; width: 13.5rem; font-size: 18px;" aria-current="page", @click=${(e)=>this.equipos()}>
                    <span class="material-symbols-outlined" id="p">
                    person_apron
                    </span> Equipos
                    </button>
                ...
                <div class=" border border-dark" style="width: 14rem; height: 33.3rem; border-radius: 1rem;">
                    <div>
                        <div class="d-flex  flex-shrink-0 p-3" style="width: 222px; background-color: rgb(201, 205, 207); border-top-left-radius: 1rem; border-top-right-radius: 1rem; height: 15rem; ">
                            ...
                            <input class="form-control font-weight-bold" id="busquedaDocumento" type="text" placeholder="Buscar por documento" @input="${this.actualizarBusquedaDocumento}" />
                        </div>
                        <div class="input-group mt-3" style="width: 12rem;">
                            <input class="form-control font-weight-bold" id="busquedaNombre" type="text" placeholder="Buscar por nombre" @input="${this.actualizarBusquedaNombre}" />
                        </div>
                        <div class="d-flex justify-content-center">
                            <button class="mt-2 text-center" style="width: 5rem; border-radius: 5px; border: rgb(250, 101, 101); background-color: rgb(250, 101, 101); color: white;" @click="${this.buscarUsuarios}">Buscar</button>
                        </div>
                ...
                <div>
                    <div class="d-flex justify-content-around ">
                        <div class=" ml-5">
                            ...
                            <select id="busquedaCampaña" @change="${this.filtrarPorCampaña}">
                            ${campañasActivas.map( (campaña) => html` ${campaña.nombre} ` )}
                            <option value="">Todas las campañas</option>
                            ${campañasActivas.map((campaña) => html`<option value="${campaña.nombre}">${campaña.nombre}</option>`)}
                        </select>                             
                        
                        
                        
                        <div class="col-sm-6 col-md-4 pt-2 border border-secondary font-weight-bold" style="width: 25rem; font-size: 20px; height: 6rem; border-radius: 7px; margin-left:5px;"><p>Contador de llamdas: <span id="contadorLlamadas"></span></p>
                                </div>
                   ...
                   <h2>Usuarios registrados:</h2>
                    <ul>
                       ${this.usuarios.map(
                    (usuario) =>
                           html`<li>${usuario.nombre} - ${usuario.email} - ${usuario.documento}</li>`
                        )}
...
<div id="myModal" class="modal">
    ...
    <span class="close" id="closeModal" @click=${this.cerrarModal}>&times;</span>
        <h2>Formulario</h2>
        ...
        ${campañasActivas.map(campaña => html`<option>${campaña.nombre}</option>`)}
      </select>
        
        <input type="submit" value="Enviar" @click=${this.guardarDatosEnLocalStorage}>
        </div>
    </div>
</div>
